...
export default class MeteorScreen extends Component {
    ...
    render() {
        if (Object.keys(this.state.meteors).length === 0) {
            ...
            var meteor_arr = Object.keys.map(this.state.meteors).map(mDate => {
                return this.state.meteors[mDate]
            })
            let meteors = [].concat.apply([], meteor_arr)
            meteors.forEach(function (e) {
                let diameter = (e.estimated_diameter.kilometers.estimated_diameter_min + e.estimated_diameter.kilometers.estimated_diameter_max) / 2
                let threatScore = (diameter / e.close_approach_data[0].miss_distance.kilometers) * 100000000
                e.threat_score = threatScore
            })
